...
export default function VenueEditPage() {
  ...
  useEffect(() => {
    async function loadVenue() {
      try {
        ...
        if (error || !venueData) {
          setErrorMessage('Lokalen kunde inte hittas')
          setLoading(false)
          return
        }
        ...
        if (venueData.status !== 'published') {
          const errors = await getPublishValidation(venueId)
  ...
  useEffect(() => {
    ...
    // Clear URL params
    if (success || error) {
      window.history.replaceState({}, '', `/dashboard/venue/${venueId}`)
    }
  }, [searchParams])
  ...
  const handlePublish = async () => {
    ...
    const result = await publishVenue(venueId)
  ...
  const handlePause = async () => {
    ...
    const result = await unpublishVenue(venueId)
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white border border-[#e7e5e4] rounded-xl p-6 mb-6">
        ...
        <VenuePhotoManager venueId={venueId} />
      </div>
